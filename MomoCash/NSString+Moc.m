...
+ (NSString *)addressWithScriptPubKey:(NSData *)script
{
    ...
    uint8_t v = MoMo_PUBKEY_ADDRESS;
    
#if MoMo_TESTNET
    v = MoMo_PUBKEY_ADDRESS_TEST;
#endif
    ...
    if (l == 5 && [elem[0] intValue] == OP_DUP && [elem[1] intValue] == OP_HASH160 && [elem[2] intValue] == 20 &&
        ...
        v = MoMo_SCRIPT_ADDRESS;
#if MoMo_TESTNET
        v = MoMo_SCRIPT_ADDRESS_TEST;
#endif
        [d appendBytes:&v length:1];
        [d appendData:elem[1]];
    }
...
#if MoMo_TESTNET
    return (version == MoMo_PUBKEY_ADDRESS_TEST || version == MoMo_SCRIPT_ADDRESS_TEST) ? YES : NO;
#endif
    
    return (version == MoMo_PUBKEY_ADDRESS || version == MoMo_SCRIPT_ADDRESS) ? YES : NO;
}
...
#if MoMo_TESNET
        return (*(const uint8_t *)d.bytes == MoMo_PRIVKEY_TEST) ? YES : NO;
#else
        return (*(const uint8_t *)d.bytes == MoMo_PRIVKEY) ? YES : NO;
#endif
    }
...
- (NSAttributedString*)attributedStringForDashSymbolWithTintColor:(UIColor*)color dashSymbolSize:(CGSize)dashSymbolSize {
    ...
    NSRange range = [attributedString.string rangeOfString:MomoCash];
